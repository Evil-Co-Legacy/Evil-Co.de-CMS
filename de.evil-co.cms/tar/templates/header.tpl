	...
	<div id="headerContainer">
		...
		<div id="userPanel" class="userPanel">
			<div class="userPanelInner">
				<p id="userNote">
					{if $this->user->userID != 0}{lang}cms.header.userNote.user{/lang}{else}{lang}cms.header.userNote.guest{/lang}{/if}
				</p>
				<div id="userMenu">
					<ul>
						{if $this->user->userID != 0}
							<li id="userMenuLogout"><a href="index.php?action=UserLogout&amp;t={@SECURITY_TOKEN}{@SID_ARG_2ND}"><img src="{icon}logoutS.png{/icon}" alt="" /> <span>{lang}cms.header.userMenu.logout{/lang}</span></a></li>
							<li id="userMenuProfileEdit"><a href="index.php?form=UserProfileEdit{@SID_ARG_2ND}"><img src="{icon}editS.png{/icon}" alt="" /> <span>{lang}cms.header.userMenu.profile{/lang}</span></a></li>
							
							{if $additionalUserMenuItems|isset}{@$additionalUserMenuItems}{/if}
	
							{if $this->user->getPermission('admin.general.canUseAcp')}
								<li id="userMenuACP"><a href="acp/index.php?packageID={@PACKAGE_ID}"><img src="{icon}acpS.png{/icon}" alt="" /> <span>{lang}cms.header.userMenu.acp{/lang}</span></a></li>
							{/if}
						{else}
							<li id="userMenuLogin" class="options"><a href="index.php?form=UserLogin{@SID_ARG_2ND}" id="loginButton"><img src="{icon}loginS.png{/icon}" alt="" id="loginButtonImage" /> <span>{lang}cms.header.userMenu.login{/lang}</span></a></li>
	
							{if !REGISTER_DISABLED}<li id="userMenuRegistration"><a href="index.php?page=Register{@SID_ARG_2ND}"><img src="{icon}registerS.png{/icon}" alt="" /> <span>{lang}cms.header.userMenu.register{/lang}</span></a></li>{/if}
	
							{if $additionalUserMenuItems|isset}{@$additionalUserMenuItems}{/if}
	
							{if $this->language->countAvailableLanguages() > 1}
								<li id="userMenuLanguage" class="languagePicker options"><a id="changeLanguage" class="hidden"><img src="{icon}language{@$this->language->getLanguageCode()|ucfirst}S.png{/icon}" alt="" /> <span>{lang}cms.header.userMenu.changeLanguage{/lang}</span></a>
		...
		<div id="header">
			 ...
			 * $searchFieldTitle=title of search input field; default=language variable cms.header.search.query
				...
				<form method="post" action="{@$searchScript}">
					...
					<div class="searchContainer">
							...
							<div class="searchInputMenu">
								<div class="hidden" id="searchInputMenu">
									<div class="pageMenu smallFont">
										<ul>
											{@$searchFieldOptions}
											{if $searchShowExtendedLink}<li><a href="{@$searchExtendedLink}{if !$searchFieldValue|empty}&amp;defaultQuery={$searchFieldValue|rawurlencode}{/if}">{lang}cms.header.search.extended{/lang}</a></li>{/if}
										</ul>
									</div>
								</div>
							</div>
						...
						{if $searchShowExtendedLink}
							<noscript>
								<p><a href="{@$searchExtendedLink}">{lang}cms.header.search.extended{/lang}</a></p>
							</noscript>
						{/if}
					</div>
				</form>
			...
			<div id="logo">
				<div class="logoInner">
					...
					{elseif $this->getStyle()->getVariable('page.logo.image.application.use') == 1}
						<a href="index.php?page=Index{@SID_ARG_2ND}" class="pageLogo">
							<img src="{@RELATIVE_CMS_DIR}images/cms-header-logo.png" title="{lang}{PAGE_TITLE}{/lang}" alt="" />
						</a>
					{/if}
				</div>
			</div>
