	...
	<div id="footerContainer">
		<div id="footer">
					...
					<ul>
						...
						{if $stylePickerOptions|count > 1}
							<li class="stylePicker{if !SHOW_CLOCK} last{/if}">
								<a id="changeStyle" class="hidden"><img src="{icon}styleOptionsS.png{/icon}" alt="" /> <span>{lang}cms.global.changeStyle{/lang}</span></a>
									...
									<form method="get" action="index.php" class="quickJump">
										<div>
											...
											<select name="styleID" onchange="if (this.options[this.selectedIndex].value != 0) this.form.submit()">
												<option value="0">{lang}cms.global.changeStyle{/lang}</option>
			...
			<p class="copyright">{lang}cms.global.copyright{/lang}</p>
		</div>
	</div>
	{if !$this->user->userID && !LOGIN_USE_CAPTCHA}
		<div class="border loginPopup hidden" id="quickLoginBox">
			<form method="post" action="index.php?form=UserLogin" class="container-1">
				...
				<p><label><input tabindex="3" type="checkbox" id="useCookies" name="useCookies" value="1" /> {lang}cms.header.login.useCookies{/lang}</label></p>
			</form>
		</div>
	{/if}
</div>